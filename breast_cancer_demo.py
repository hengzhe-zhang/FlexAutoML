...
from data_preprocessing.categorical_encoding import (
...
from machine_learning.hyper_parameter_optimization import (
    objective_catboost,
    objective_lgbm,
...
if __name__ == "__main__":
    ...
    X_train, X_test = feature_selection_lgbm(
        X_train, y_train, X_test, percentage_total_weight=0.95
    )

    # Perform hyperparameter optimization for LightGBM
    lgbm_best_params = hyperparameter_optimization(
        X_train, y_train, n_trials=3, function=objective_lgbm
    )

    # Perform hyperparameter optimization for CatBoost
    catboost_best_params = hyperparameter_optimization(
        X_train, y_train, n_trials=3, function=objective_catboost
    )

    # Train models
    lgbm_clf, catboost_clf = train_model(
        X_train, y_train, lgbm_best_params, catboost_best_params
    )
